...
function star_cloudprnt_settings() {
	...
	add_settings_field("star-cloudprnt-day-end", "Day End Report", "star_cloudPRNT_print_day_end_report_button", "star_cloudprnt_setup", "star_cloudprnt_design_section");
}
...
function star_cloudPRNT_print_day_end_report_button() {
?>
	<button type="button" class="button" onclick="print_day_end_report()">Print Day End Receipt</button>
	<div id="day_end_result"></div>
	<script>
		function print_day_end_report() {
			$.post('<?php echo admin_url('admin-ajax.php?action=star_cloudprnt_print_end_of_day_report') ?>').then(function() {
				$("#day_end_result").html("Success. Report will print momentarily");
				setTimeout(function() {
					$("#day_end_result").html("");
				}, 5000);
			});
